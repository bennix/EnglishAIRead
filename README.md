...
- **新增填空练习**：上海选词填空（250–350 词、11 选 10、不变词形）与完形填空（250–300 词、15 或 20 空、每空四选一）。按当前文章改编，逐空判分并提供中文解析，各题型分别保存进度并支持历史导出。
...
- **长任务流式响应**：出题、批阅、查词与追问采用流式接收，最长等待 10 分钟，所有模型提供 24,000 token 输出预算。AI 追问实时显示回答，完成后渲染 Markdown；出题实时显示短文和题干，完成后组装为可作答题目。
